...
"""
def generate_list_of_parameters ():
...
def create_run_folders(augmentation_d, normalization_d, prj_path, trainHistory_subname):
    ...
    run_name = cfg.model_name + cfg.run_name_custom_string + '_w' + \
               str(cfg.cnn_settings_d['weights']) + '_unfl' + str([freeze["unfreezed_layers_perc"] for freeze in cfg.list_all_param_combinations]) + \
               '_d' + str([dropout["dropout_top_layers"] for dropout in cfg.list_all_param_combinations]) + '_lr' + str([lr["learning_rates"] for lr in cfg.list_all_param_combinations])
...
def optimizer_loading():
    ...
    if cfg.optimizer == "SGD":
        ...
        optimizer = tf.keras.optimizers.SGD(learning_rate=[lr["learning_rates"] for lr in cfg.list_all_param_combinations], momentum=cfg.momentum, nesterov=False,
                                            name="SGD")
    elif cfg.optimizer == "Adam":
        # from transferLearning.py
        optimizer = optimizers.Adam(lr=[lr["learning_rates"] for lr in cfg.list_all_param_combinations], beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
    else:
...
def model_loader():
    ...
    if cfg.cnn_settings_d['include_top'] is False and cfg.add_custom_top_layers:
        base_model = nnm.add_classification_top_layer(base_model, cfg.cnn_settings_d['classes'], cfg.neurons_l,
                                      [dropout["dropout_top_layers"] for dropout in cfg.list_all_param_combinations],
                                      [freeze["unfreezed_layers_perc"] for freeze in cfg.list_all_param_combinations])
        print('Final Model with added top layers', type(base_model), 'layers', len(base_model.layers),
              'of which', round(len(base_model.layers) * [freeze["unfreezed_layers_perc"] for freeze in cfg.list_all_param_combinations] / 100), 'are unfroozen or ',
